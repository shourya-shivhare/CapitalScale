...
export default function BankAdminLoginPage() {
  ...
  const { loginBank, verifyMfa, isLoading } = useAuth();
  ...
  const onSubmit = async (data) => {
    setServerError('');
    try {
      const result = await loginBank(data);
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-5">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-center text-white placeholder-slate-500 text-lg tracking-widest font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500/50 transition-all"
                  required
                />
              </div>
